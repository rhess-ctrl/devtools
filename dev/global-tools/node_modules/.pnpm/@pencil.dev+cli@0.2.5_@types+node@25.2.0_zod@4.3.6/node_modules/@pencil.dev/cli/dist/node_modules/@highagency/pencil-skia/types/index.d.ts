...
export interface PDFDocument extends EmbindObject<"PDFDocument"> {
  /**
   * Returns a canvas for drawing a new page with the given dimensions in points.
   * Call endPage() when done drawing on this page.
   */
  beginPage(width: number, height: number): Canvas;

  /**
   * Finishes the current page. Call beginPage() to start a new one.
   */
  endPage(): void;

  /**
   * Finalizes the PDF document and returns the encoded bytes.
   */
...
export interface Typeface extends EmbindObject<"Typeface"> {
  ...
  /**
   * Return the variation design parameters (axes) for this typeface.
   * Returns an empty array for non-variable fonts.
   */
  getVariationDesignParameters(): Array<{
    tag: number;
    min: number;
    def: number;
    max: number;
...
export interface FontCollection extends EmbindObject<"FontCollection"> {
  ...
  setDynamicFontManager(fontManager: TypefaceFontProvider | null): void;
  clearCaches(): void;
}
...
export interface PathBuilderConstructor
  extends DefaultConstructor<PathBuilder> {
  /**
